...
import info.nightscout.androidaps.databinding.ExtAppCommunicatorFragmentBinding
...
class ExternalAppCommunicatorFragment : DaggerFragment() {
    ...
    private var _binding: ExtAppCommunicatorFragmentBinding? = null
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        _binding = ExtAppCommunicatorFragmentBinding.inflate(inflater, container, false)
        return binding.root
    ...
    fun updateGui() {
        ...
        for (x in start until smsCommunicatorPlugin.messages.size) {
            ...
            when {
                ...
                sms.sent     -> {
                    val broadcastText = if (sms.broadcast) " (broadcast)" else ""
                    logText += dateUtil.timeString(sms.date) + " &gt;&gt;&gt; " + (if (sms.processed) "● " else "○ ") + sms.phoneNumber + broadcastText + " <b>" + sms.text + "</b><br>"
                }
            }
        }
