...
@OpenForTesting
@Singleton
class ExternalAppCommunicatorPlugin @Inject constructor(
    ...
    .fragmentClass(ExternalAppCommunicatorFragment::class.java.name)
    .pluginIcon(R.drawable.ic_sms)
    .pluginName(R.string.ext_app_communicator)
    .shortName(R.string.ext_app_communicator_short_name)
    .preferencesId(R.xml.pref_ext_app_communicator)
    .description(R.string.ext_app_communicator_description),
    ...
    class ExternalAppCommunicatorWorker(
        context: Context,
        params: WorkerParameters
    ) : Worker(context, params) {

        @Inject lateinit var smsCommunicatorPlugin: ExternalAppCommunicatorPlugin
        ...
        override fun doWork(): Result {
            val user = inputData.getString("user")
                ?: return Result.failure(workDataOf("Error" to "missing user in input data"))
            val text = inputData.getString("text")
                ?: return Result.failure(workDataOf("Error" to "missing text in input data"))
            val message = Sms(user, text, true)
            smsCommunicatorPlugin.processSms(message)
    ...
    fun processSms(receivedSms: Sms) {
        if (!isEnabled(PluginType.GENERAL)) {
            aapsLogger.debug(LTag.SMS, "Ignoring messages. Plugin disabled.")
            return
        }
    ...
    private fun sendSMSToAllNumbers(sms: Sms) {
        sendSMS(Sms.broadcast(sms))
    }

    fun sendSMS(sms: Sms): Boolean {
        sms.text = stripAccents(sms.text)
        aapsLogger.debug(LTag.SMS, "Sending intent to " + sms.phoneNumber + ": " + sms.text)
        val sendIntent: Intent = Intent().apply {
            action = Intents.EXTERNAL_RESPONSE_SENT
            putExtra("user", sms.phoneNumber)
            putExtra("text", sms.text)
            putExtra("broadcast", sms.broadcast)
        }
